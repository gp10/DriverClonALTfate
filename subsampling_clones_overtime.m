function [clonesizes_sampled,clonesizes_ref_sampled] = subsampling_clones_overtime(nsamples,sizesamples,clonesizes,clonesizes_ref,filter,timepoints)
%% Repeated sampling (random permutation) of simulated clones into subsets:
...
% clonesizes_ref_sampled: cell array {subsets,1}{1,timepoints}(:,1) of basal clone sizes in each subset at each specified time point
...
for luptime = 1:size(timepoints,2)
    ...
    for rnd_loop = 1:nsamples
        for rnd_loop2 = 1:sizesamples(luptime)
            ...
            clonesizes_sampled{rnd_loop,1}{1,luptime}(rnd_loop2,1) = clonesizes(rnd_pickpos,luptime);
            clonesizes_ref_sampled{rnd_loop,1}{1,luptime}(rnd_loop2,1) = clonesizes_ref(rnd_pickpos,luptime);
        end
    end
end
