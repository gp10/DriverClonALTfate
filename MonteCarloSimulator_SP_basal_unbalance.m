function [nx_basal,ntime] = MonteCarloSimulator_SP_basal_unbalance(rtime,lambda,r,Delta,gamma,indiv,tlag,GamShape)
...
% mutant progenitor with unbalanced fates. It allows to consider
% gamma-distributed cell cycle periods.
...
% tlag = 0.5/7;
% GamShape = 8;
% [nx_basal,ntime] = MonteCarloSimulator_SP_basal_unbalance(rtime,lambda,r,Delta,gamma,indiv,tlag,GamShape);

%% Default parameter values:
tic
% Initial error checks:
assert(nargin>4,'Single-Progenitor simulator function requires at least five inputs.')
assert((r>=0 & r<= 0.5),'Unvalid value for r. Should fall between 0-0.5')
assert((Delta>=0 & Delta<= 1),'Unvalid value for Delta. Should fall between 0-1')
if (nargin < 8)
    indiv=100;
    tlag = 0;
    GamShape = 1;
