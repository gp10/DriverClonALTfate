function plot_AvgCloneSize_simulated(timepoints,avgCloneSize,showCI,FigProp)
%% Plots the simulated average clone size over time
% The time course of the average (basal or total) clone size from the
% simulations is plotted with or without confidence bounds (if provided).
...
% FigProp: structure containing output display properties
    % struct{Color=='r', Leyend=='CTL', data=='Basal cells / clone'}
...
% showCI = 1;
% FigProp.Color = 'r'; FigProp.Leyend = 'CTL'; FigProp.data = 'average basal cells / clone';
% plot_AvgCloneSize_simulated(timepoints,avgCloneSize,showCI,FigProp);
...
hold on
plot(timepoints.*7,avgCloneSize.avg,'Color',FigProp.Color)
if showCI == 1
    plot(timepoints.*7,avgCloneSize.ci95up,'Color',FigProp.Color)
    plot(timepoints.*7,avgCloneSize.ci95dn,'Color',FigProp.Color)
end
xlabel('Time (days)'); ylabel(FigProp.data)
