function [nx_basal,nx_total,ntime] = MonteCarloSimulator_SP_total_unbalance_varMu(rtime,lambda,dens,r,Delta,gamma,micetime,mice_m_t,indiv,tlag,GamShape)
%% Non-Markovian Monte Carlo simulator of unbalanced Single Progenitor (SP) model dynamics
% Total clone sizes are simulated over time starting from a single labelled
% progenitor. It allows to consider gamma-distributed cell cycle periods
% and a time-dependent shedding rate (mu).
...
% r: default probability of symmetric division leading to two progenitors, PP, or two differentiating cells, DD
% Delta: fate imbalance favoring PP vs. DD outcome
...
% mice_m_t = [0.6014 0.5616 0.5735 0.4430];
% indiv = 1000;
% tlag = 0.5/7;
% GamShape = 8;
% [nx_basal,nx_total,ntime] = MonteCarloSimulator_SP_total_unbalance_varMu(rtime,lambda,dens,r,Delta,gamma,micetime,mice_m_t,indiv,tlag,GamShape);
...
for it=1:indiv
    ...
    %  P  D  S
    x=[1  0  0]; % a single-cell clone with just one progenitor cell (P).
    ...
    while Id_Cell <= size(x,1)
        ...
        if tini(Id_Cell,1) <= timelim
            ...
            mu = (gamma*(1-dens)*(1+m))/m - lambda*dens; % week-1

            % Calculation of single-event probabilities:
            p(1) = lambda*r*(1+Delta)*x(Id_Cell,1); % P -> P + P
            p(2) = lambda*(1-2*r)*x(Id_Cell,1); % P -> P + D
            p(3) = lambda*r*(1-Delta)*x(Id_Cell,1); % P -> D + D
