function [nx_basal,nx_total,ntime] = MonteCarloSimulator_SP_total_unbalance_varMu_evenStart(rtime,lambda,dens,r,Delta,gamma,micetime,mice_m_t,indiv,tlag,GamShape)
...
% tlag = 0.5/7;
% GamShape = 8;
% [nx_basal,nx_total,ntime] = MonteCarloSimulator_SP_total_unbalance_varMu_evenStart(rtime,lambda,dens,r,Delta,gamma,micetime,mice_m_t,indiv,tlag,GamShape);
...
for it=1:indiv
    ...
    while Id_Cell <= size(x,1)
        ...
        if tini(Id_Cell,1) <= timelim
            ...
            p(3) = lambda*r*(1-Delta)*x(Id_Cell,1); % P_-> D + D
